...
## LayerX 飛田和裕 — 敵対的レビューループの実装（2026-07-22登壇）
- 「"惜しいデザイン"を生み出さないためのAIレビューループ」: デザイン前段（PRD/Spec）の観点抜け漏れをAIレビューで潰し手戻りを減らす実装。素材/2026-07-22_LayerX_惜しいデザインAIレビューループ.pdf
- **敵対的レビューエージェント**: PRD作成や実装と**コンテクストを共有せず「粗探し」だけを責務**とする別エージェント。1個でもFAILなら該当フェーズへ差し戻し、PASSまでループ — 本パックの独立サブエージェント方式・A照合差し戻しと同型。
- 実装: 5フェーズ（チケット→PRD→Spec→ワイヤーフレーム→デザイン）に Build/Review を交互配置（prd-builder / prd-adversarial-reviewer / /build-spec-design / /review-design-wireframe）。Notion→Storybook→Figma連携。
- **「防げる手戻り」と「健全な手戻り」の区別**（最重要の輸入概念）: Phase1-3の仕様起因の手戻りは防げる/防ぐべき。Phase4-5の手戻りは**デザインシステム拡張につながる健全なもの** — 手戻りゼロは目標ではない。
- 手戻り2分類: 「画面ができあがる**前に**わかるもの」（権限差分・API schema・件数レンジ0/1/N・UI Stack等）vs「できて**初めて**わかるもの」（DS準拠・ライティング・実データの手触り・a11y・パフォーマンス）— 本パックのclass A（事前照合可能）/ B・C（見て判定）の分類を手戻り視点から裏付け。
- 課題認識も同じ: 「精度は極めて高いがトークン膨大・時間がかかる」「**レビューが回るたびに学びが積み上がり精度が自動向上するのが理想**（以前も同じ箇所でFAILしたな〜）」— 本パックのcounters/側面タグ・原因束ねが既にその実装。
- 「.mdやSkillsといったハーネスを丁寧に作っても忠実に守るとは限らない。ノーガードでは意図通りにならない」— ハーネス＋検証ループの二段が必須という結論も一致。
