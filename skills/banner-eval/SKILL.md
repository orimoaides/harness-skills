...
### 2. class B — 定性判定（10段階・4帯）
評価セットのB観点を1つずつ:
`{ 観点ID, score: 1-10, band: fail(1-3)|rework(4-5)|polish(6-7)|pass(8-10), 根拠1-2文, 測定手段: 実測|目視|読解, 確信度: high|mid|low }`
- **帯を先に決めてから帯内で1点刻み**。fail=致命（出せない）／rework=要改善・構造（組み直しが要る）／polish=要改善・表層（仕上げで直る）／pass=基準を満たす。「直すとしたら作り直しか、仕上げか」でrework/polishを分ける。較正は帯一致で見るため帯の判断が本体。
...
- **class C観点も10段階の影スコアを出す**（合計には入れない）。観察事実の列挙とセットで人間の判定材料に。苦手観点こそ判定を繰り返さないと較正が始まらない。
