...
### 2. class B — 定性判定（10段階）
評価セットのB観点を1つずつ:
`{ 観点ID, score: 1-10, band: fail(1-3)|caution(4-7)|pass(8-10), 根拠1-2文, 測定手段: 実測|目視|読解, 確信度: high|mid|low }`
- **帯を先に決めてから帯内で1点刻み**（1–3=致命／4–7=要改善／8–10=基準を満たす）。較正は帯一致で見るため帯の判断が本体。
- 迷ったら低い帯に落とさず **4–7帯＋確信度low**（過剰な低帯は誤指摘カウントを濁らせる）。
