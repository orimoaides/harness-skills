...
flowchart TD
  ...
  C1 --> C2["class B — 10段階採点（4帯: 致命1-3 / 構造4-5 / 表層6-7 / 合格8-10）"]
...
**クイックスタート（brand-profileが空でも回せる最小構成）**: 初回からフル評価をするとA照合の大半が「未確認」で埋まり第一印象が悪い。最初は ①画像をファイルで渡す ②訴求1位だけ口頭で指定 ③評価セット「標準」— この3つで回すと、実測系A照合＋B主要観点だけの見栄えするレポートになる。規定・原稿・brand-profileは較正を回しながら順次足せばよい（足りない物はレポートの未確認テーブルが教えてくれる）。
